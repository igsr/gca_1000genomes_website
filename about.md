...
Sequencing was too expensive to deeply sequence the many samples being studied in the project. However, any particular region of the genome generally contains a limited number of haplotypes. Data was combined across samples to allow efficient detection of most of the variants in a region. The Project planned to sequence each sample to about 4X coverage; at this depth sequencing could not provide the complete genotype of each sample, but can allow the detection of most variants with frequencies as low as 1%. The data from 2504 samples was combined to allow highly accurate estimation (imputation) of the variants and genotypes for each sample, even at locations not seen directly by the low coverage sequencing.
...
{% endcapture %}
{% include section_md.html content=data_use %}
...
{% endcapture %}
{% include section_md.html content=samples %}
